defmodule Typing.Editor.GameEditor do
  ...
  def update(%__MODULE__{} = editor, "select_mode", %{"mode" => "training"}) do
    char_list = Game.get_themes() |> Enum.shuffle() |> Enum.chunk_every(10) |> hd()
    display_char = hd(char_list) |> IO.inspect()
    char_count = String.length(display_char.theme)
    %{
      editor
      | mode: :training,
  ...
  defp next_char(editor, key) do
    char_list = List.delete(editor.char_list, editor.display_char) |> IO.inspect()
